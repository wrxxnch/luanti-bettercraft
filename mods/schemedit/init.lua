...
local function set_item_metadata(itemstack, prob, force_place)
	...
	elseif force_place == false then
		smeta:set_string("schemedit_force_place", "")
	end
...
schemedit.add_form("main", {
	...
	handle = function(self, pos, name, fields)
		...
		local metafields = realmeta:to_table().fields
		local hashpos = minetest.hash_node_position(pos)
		...
		if (fields.x and fields.x ~= "") then
			...
			if x then
				metafields.x_size = tostring(math.max(x, 1))
			end
		end
		if (fields.y and fields.y ~= "") then
			local y = tonumber(fields.y)
			if y then
				metafields.y_size = tostring(math.max(y, 1))
			end
		end
		if (fields.z and fields.z ~= "") then
			local z = tonumber(fields.z)
			if z then
				metafields.z_size = tostring(math.max(z, 1))
			end
		end

		-- Save schematic name
		if fields.name then
			metafields.schem_name = fields.name
		end

		-- Check schematic name for forbidden characters
		if not check_schem_name(metafields.schem_name or "") then
		...
		if fields.border then
			if metafields.schem_border == "true" and schemedit.markers[hashpos] then
				schemedit.unmark(pos)
				metafields.schem_border = "false"
			else
				schemedit.mark(pos)
				metafields.schem_border = "true"
			end
		end

		-- Export schematic
		if fields.export and metafields.schem_name and metafields.schem_name ~= "" then
			local pos1, pos2 = schemedit.size(pos)
			pos1, pos2 = schemedit.sort_pos(pos1, pos2)
			local path = export_path_full .. DIR_DELIM
			minetest.mkdir(path)

			local slist = minetest.deserialize(metafields.slices)
			local slice_list = {}
			...
			local after_emerged = function(blockpos, action, calls_remaining, param)
				if action == minetest.EMERGE_CANCELLED then
					minetest.chat_send_player(param.player_name, minetest.colorize(TEXT_COLOR_ERROR,
						...
						minetest.pos_to_string(param.pos1),
						minetest.pos_to_string(param.pos2))))
			...
			local emerge_param = {
				...
				pos2 = pos2,
				path = path,
				schem_name = metafields.schem_name,
				player_name = name,
		...
		-- Import schematic
		if fields.import and metafields.schem_name and metafields.schem_name ~= "" then
			...
			local filepath = path..metafields.schem_name..".mts"
			local schematic = minetest.read_schematic(filepath, {write_yslice_prob="low"})
			local success = false

			if schematic then
				metafields.x_size = tostring(schematic.size.x)
				metafields.y_size = tostring(schematic.size.y)
				metafields.z_size = tostring(schematic.size.z)
				metafields.slices = minetest.serialize(renumber(schematic.yslice_prob))
				local special_x_size = schematic.size.x
				local special_y_size = schematic.size.y
				local special_z_size = schematic.size.z

				if node.param2 == 1 then
					pos1 = vector.add(pos, {x=1,y=0,z=-schematic.size.z+1})
					metafields.x_size, metafields.z_size = metafields.z_size, metafields.x_size
				elseif node.param2 == 2 then
					pos1 = vector.add(pos, {x=-schematic.size.x+1,y=0,z=-schematic.size.z})
				elseif node.param2 == 3 then
					pos1 = vector.add(pos, {x=-schematic.size.x,y=0,z=0})
					metafields.x_size, metafields.z_size = metafields.z_size, metafields.x_size
				else
		...
		realmeta:from_table({fields = metafields, inventory = inv})

		-- Update border
		if not fields.border and metafields.schem_border == "true" then
			schemedit.mark(pos)
		end
...
S("The node HUD is not updated automatically and may be outdated. The node HUD only updates the HUD for nodes close to you whenever you place the tool or press the punch and sneak keys simultaneously. If you sneak-punch a schematic creator, then the node HUD is updated for all nodes within the schematic creator's region, even if this region is very big."),
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		if not prob or prob == 255 then
			nmeta:set_string("schemedit_prob", "")
		else
		...
		if force_place == "true" then
			...
			nmeta:set_string("schemedit_force_place", "")
		end
...
local parse_command_arguments = function(arguments, relative_pos)
...
minetest.register_chatcommand("placeschem", {
	...
	func = function(name, param)
		...
		if minetest.read_schematic then
			...
			-- Legacy support for engine versions that do not have minetest.read_schematic.
