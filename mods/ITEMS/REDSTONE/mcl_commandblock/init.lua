...
local function resolve_relative_coords(param, pos)
    ...
    param = param:gsub("~([%-]?[%d%.]*)", function(offset)
        ...
        local axis = axis_cycle[axis_index] or "x"
        axis_index = axis_index + 1
        if axis_index > 3 then
            axis_index = 1
        end

        local base = tonumber(pos[axis]) or 0
...
local function execute_commandblock(pos)
    ...
    if meta:get_int("conditional") == 1 then
        ...
        local ok, result = pcall(get_success_behind, pos)
    ...
    local commander = meta:get_string("commander") or ""
    local commands = meta:get_string("commands") or ""

    commands = resolve_commands(commands, pos)
    ...
    for _, command in pairs(commands:split("\n")) do
        ...
        if command and command ~= "" then
            ...
            if cmddef and cmddef.func then
                ...
                local ok, success, msg = pcall(function()
                    ...
                    return cmddef.func(commander, param)
    ...
    if dir then
        ...
        if front_pos then
            ...
            if front_node and front_node.name and front_node.name:find("chain") then
                ...
                if front_meta then
                    ...
                    if front_auto or front_powered then
                        ...
                        pcall(execute_commandblock, front_pos)
