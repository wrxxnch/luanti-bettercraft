...
local on_place = mcl_util.generate_on_place_plant_function(function(place_pos, _)
    local soil_node = core.get_node_or_nil({
        x = place_pos.x,
        y = place_pos.y - 1,
        z = place_pos.z
    })
    ...
    return (core.get_item_group(snn, "supports_mushrooms") == 1 or
               (light_ok and core.get_item_group(snn, "solid") == 1 and core.get_item_group(snn, "opaque") == 1))
end)

local longdesc_intro_brown = S(
...
local function on_bone_meal(_, _, _, pos, n)
    ...
    if bn ~= "mcl_core:mycelium" and bn ~= "mcl_core:dirt" and core.get_item_group(bn, "grass_block") ~= 1 and bn ~=
        "mcl_core:coarse_dirt" and bn ~= "mcl_core:podzol" then
        return
    end
    ...
    for i = 0, base_height do
        core.set_node(vector.offset(pos, 0, i, 0), {
            name = stem
        })
    end
    return ok ~= nil
...
core.register_node("mcl_mushrooms:mushroom_brown", {
    ...
    on_use = minetest.item_eat(2), -- 🍄 COMESTÍVEL
...
core.register_node("mcl_mushrooms:mushroom_red", {
    ...
    on_use = function(itemstack, user, pointed_thing)
        ...
        -- executa função de comer corretamente
        return core.item_eat(2)(itemstack, user, pointed_thing)

        -- se quiser dano ou veneno, coloque aqui depois
    end,
    ...
    core.register_on_item_eat(function(_, _, itemstack, user, _) -- poisoning effect
            if itemstack:get_name() == "mcl_mushrooms:mushroom_red" then
...
core.register_abm({
    ...
    action = function(pos, node)
        ...
        local rnd = vector.new(selected_pos.x + math.random(-1, 1),
            selected_pos.y + math.random(0, 1) - math.random(0, 1), selected_pos.z + math.random(-1, 1))
        ...
        local rnd2 = vector.new(rnd.x + math.random(-1, 1), rnd.y, rnd.z + math.random(-1, 1))
        ...
        if core.get_node_light(rnd2, 0.5) > 12 or (core.get_item_group(node_under.name, "opaque") == 0) or
            (core.get_item_group(node_under.name, "solid") == 0) then
            return
        end
