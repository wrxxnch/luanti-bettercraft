...
mcl_player.register_globalstep(function(player)
	...
	if on_walk_through[mcl_player.players[player].nodes.feet] then
		...
		on_walk_through[mcl_player.players[player].nodes.feet](npos, core.get_node(npos), player)
	end
	if on_walk_through[mcl_player.players[player].nodes.head] then
		local npos = vector.add(pos, mcl_player.node_offsets.head)
		on_walk_through[mcl_player.players[player].nodes.head](npos, core.get_node(npos), player)
	end
end)
