...
local function set_commandblock_result(success)
    ...
    local pos =
        core.commandblock_pos
        or (minetest.get_commandblock_pos and minetest.get_commandblock_pos())
...
local function parse_coord(str,current)
    ...
    if not str then return current end
...
local function get_pos_from_args(args,player)
    ...
    return vector.round({x=x,y=y,z=z}),args
...
minetest.register_chatcommand("testfor",{
    ...
    func=function(name,param)
        ...
        local exec_if,exec_unless,param=
            extract_conditional_commands(param)
...
local src=minetest.get_node({x=x,y=y,z=z})
