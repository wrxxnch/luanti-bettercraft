-- bc_mc_name_resolver.lua
-- Utilitário for resolver nomes estilo Minecraft no Mineclonia / Luanti
-- Allows use of: stone, dirt, zombie, etc without namespace
-- Compatible with Luanti / Minetest 5.14+
...
function M.resolve_node(name)
	...
	-- 2) se já existir como node, returns
	if core.registered_nodes[name] then
		return name
	end

	-- 3) se not tiver namespace, procurar pelo nome curto
...
function M.resolve_item(name)
	...
	-- 2) se já existir como item, returns
