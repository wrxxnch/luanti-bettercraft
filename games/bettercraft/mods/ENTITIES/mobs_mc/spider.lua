...
-- Spider by AspireMint
-- fishyWET (CC-BY-SA 3.0 license for texture)
...
-- CORREÇÃO:
-- Algumas versões do mcl_mobs não possuem slowdown_nodes.
-- Nesse caso, usamos uma tabela vazia para evitar o erro:
-- "attempt to index field 'slowdown_nodes' (a nil value)"

spider.slowdown_nodes = table.copy(mob_class.slowdown_nodes or {})

-- Teias não devem desacelerar a aranha.
...
function spider:gopath_internal(target, speed_bonus, animation, tolerance, penalties)
	...
	local rc = mob_class.gopath_internal(
		self,
		target,
		speed_bonus,
		animation,
	...
	if rc then
		self._gopath_destination = mcl_util.get_nodepos(target)
		return rc
	else
		return nil
	end
...
-- Prevent animations from being reset if an obstruction is being climbed.
...
function spider:navigation_step(dtime, moveresult)
	...
	if self:navigation_finished() then
		...
		if self._gopath_destination then
			...
			local bb_width =
				(self.collisionbox[4] - self.collisionbox[1]) / 2

			local dist_xz =
				math.sqrt(dx * dx + dz * dz)

			if (
				(self_pos.y <= dest.y and dist_xz > bb_width / 2)
				or dist_xz > bb_width
			) then
...
local function mc_light_value(self)
	...
	brightness =
		(core.get_node_light(pos) or 0) / 15.0

	value =
		brightness / (4 - 3 * brightness)
...
function spider:attack_melee(
	...
	local dist =
		vector.distance(self_pos, target_pos)

	local chance =
		math.round(5 * dtime / 0.05)

	-- Prevent math.random(0)
	if chance < 1 then
		chance = 1
	end
...
function spider:should_continue_to_attack(target)
	...
	if math.random(100) == 1
		and mc_light_value(self) >= 0.5 then

		return false
	end

	return mob_class.should_continue_to_attack(
		self,
		target
	)
end
...
function spider:should_attack(target)
	...
	return mob_class.should_attack(
		self,
		target
	)
...
function cave_spider:on_spawn()
	...
	-- Cave spiders cannot receive special status effects
	-- or spawn as jockeys.

end

------------------------------------------------------------------------
-- Register Cave Spider.
------------------------------------------------------------------------
...
local spider_spawner = table.merge(
	mobs_mc.monster_spawner,
	{
		name = "mobs_mc:spider",
...
local cave_spider_spawner = table.merge(
	mobs_mc.monster_spawner,
	{
