...
bc_blockframe = {}
bc_blockframe.active = {} -- [name] = { type="single"|"composite", entities={}, ... }
bc_blockframe.memory = {}
bc_blockframe.del_history = {}
bc_blockframe.world_path = minetest.get_worldpath()

--load gizmos
dofile(minetest.get_modpath("bc_blockframe") .. "/gizmo.lua")

--------------------------------------------------
-- HELP
--------------------------------------------------
function bc_blockframe.help_text()
    return [[
...
/bc_blockframe <args>
/bc_blockframe_set
/bc_blockframe_cancel
/bc_blockframe_undo

/bc_blockframe_del radius=N
/bc_blockframe_del_undo

/bc_blockframe_apply radius=N <args>
/bc_blockframe_undo_apply

/bc_blockframe_save <nome> radius=N
/bc_blockframe_load <nome> <args>
...
pos=x,y,z         position absoluta ou offset
...
radius=N          raio ao redor do player

Exemplo:
 /bc_blockframe_apply radius=10 size=2
 → altera todos bc_blockframes próximos
...
/bc_blockframe size=0.5 rotate=0,45,0
/bc_blockframe_apply radius=5 glow=10
/bc_blockframe_load minha_casa size=2 rotate=0,90,0
...
-- PARSER
--------------------------------------------------
function bc_blockframe.parse_args(param)
	local args = {}
...
-- Shared function to apply arguments to any bc_blockframe entity
local function shared_apply_args(self, args, global_args)
	...
	-- Merge with global_args (from /bc_blockframe_load)
	if global_args then
...
-- ENTIDADES
--------------------------------------------------
minetest.register_entity("bc_blockframe:preview", {
	initial_properties = {
	...
	on_step = function(self)
		...
		local active = bc_blockframe.active[self.player_name]
		if not active then return end
	end
})

minetest.register_entity("bc_blockframe:placed", {
	initial_properties = {
...
minetest.register_globalstep(function(dtime)
	for name, data in pairs(bc_blockframe.active) do
...
-- SPAWN HELPERS
--------------------------------------------------
function bc_blockframe.clear_active(name)
	if bc_blockframe.active[name] then
		for _, obj in ipairs(bc_blockframe.active[name].entities) do
			if obj:get_luaentity() then obj:remove() end
		end
		bc_blockframe.active[name] = nil
	end
end
...
-- COMANDOS
--------------------------------------------------
minetest.register_chatcommand("bc_blockframe",{
	func=function(name,param)
		local player = minetest.get_player_by_name(name)
		if not player then return end
		local args = bc_blockframe.parse_args(param)

		bc_blockframe.clear_active(name)

		local node = get_wielded_item(player)
		if not node and bc_blockframe.memory[name] then node=bc_blockframe.memory[name].node end
		if not node then return false, "Hold a block or use a previous one." end

		local obj = minetest.add_entity(player:get_pos(), "bc_blockframe:preview", minetest.serialize({node=node, player_name=name}))
		if obj then
			local ent = obj:get_luaentity()
			ent:apply_args(args)
			bc_blockframe.active[name] = {
				type = "single",
				entities = {obj},
...
minetest.register_chatcommand("bc_blockframe_load", {
	params = "<nome> [args]",
	func = function(name, param)
		local filename, args_str = param:match("^(%S+)%s*(.*)$")
		if not filename then return false, "Usage: /bc_blockframe_load <nome> [args]" end

		local filepath = bc_blockframe.world_path .. "/" .. filename .. ".bf"
		local file = io.open(filepath, "r")
		if not file then return false, "File not found." end
		local data = minetest.deserialize(file:read("*all"))
		file:close()

		if not data or not data.entities then return false, "File invalid." end

		bc_blockframe.clear_active(name)
		local global_args = bc_blockframe.parse_args(args_str)
		local preview_objs = {}
		...
		for _, e in ipairs(data.entities) do
			...
			local obj = minetest.add_entity(player:get_pos(), "bc_blockframe:preview",
		...
		return true, "Preview loaded (centered). Use /bc_blockframe_set for confirm."
	end
})

minetest.register_chatcommand("bc_blockframe_set",{
	func=function(name)
		local data = bc_blockframe.active[name]
		if not data then return false, "No active preview." end
		...
		for _, obj in ipairs(data.entities) do
			...
			if ent then
				...
				final_args.pos = pos
				minetest.add_entity(pos, "bc_blockframe:placed", minetest.serialize({node=ent.node, args=final_args}))

				bc_blockframe.memory[name] = {node=ent.node, args=final_args, pos=pos}
				count = count + 1
			end
		end

		bc_blockframe.clear_active(name)
		return true, "Confirmed: " .. count .. " block(s) colocado(s)."
	end
})

function bc_blockframe.save_map(filename, center_pos, radius)
	...
	local file = io.open(bc_blockframe.world_path .. "/" .. filename .. ".bf", "w")
	if file then file:write(minetest.serialize(data)); file:close(); return true, #data.entities end
	return false, "Error ao salvar."
end

minetest.register_chatcommand("bc_blockframe_save", {
	params = "<nome> [radius=N]",
	func = function(name, param)
		local filename, args_str = param:match("^(%S+)%s*(.*)$")
		if not filename then return false, "Usage: /bc_blockframe_save <nome> [radius=N]" end
		local args = bc_blockframe.parse_args(args_str)
		local radius = tonumber(args.radius) or 10
		local player = minetest.get_player_by_name(name)
		local success, count = bc_blockframe.save_map(filename, player:get_pos(), radius)
		if success then return true, "Saved: " .. count .. " blocks." else return false, count end
	end
})

minetest.register_chatcommand("bc_blockframe_cancel",{
	func=function(name)
		if not bc_blockframe.active[name] then return false, "No preview to cancel." end
		bc_blockframe.clear_active(name)
		return true, "Preview canceled."
	end
})

minetest.register_chatcommand("bc_blockframe_undo",{
	func=function(name)
		local mem = bc_blockframe.memory[name]
		if not mem then return false,"No block for desfazer." end
		local objs = minetest.get_objects_inside_radius(mem.pos, 0.5)
		for _,obj in ipairs(objs) do
			local luaent = obj:get_luaentity()
			if luaent and luaent.name=="bc_blockframe:placed" then obj:remove(); break end
		end
		bc_blockframe.memory[name]=nil
		return true,"Desfeito."
	end
})

minetest.register_chatcommand("bc_blockframe_del", {
	func = function(name, param)
		local player = minetest.get_player_by_name(name)
		local args = bc_blockframe.parse_args(param)
		...
		bc_blockframe.del_history[name] = removed_list
		return true, "Removed " .. #removed_list .. " blocks."
	end
})

minetest.register_chatcommand("bc_blockframe_del_undo", {
	func = function(name)
		local history = bc_blockframe.del_history[name]
		if not history then return false, "Nada for desfazer." end
		for _, item in ipairs(history) do
			minetest.add_entity(item.pos, "bc_blockframe:placed", minetest.serialize({node=item.node, args=item.args}))
		end
		bc_blockframe.del_history[name] = nil
		return true, "Restaurado."
	end
})

minetest.register_chatcommand("bc_blockframe_apply", {
	func = function(name, param)
		local player = minetest.get_player_by_name(name)
		local args = bc_blockframe.parse_args(param)
		...
		return true, "Applied to " .. count .. " blocks."
	end
})

minetest.register_chatcommand("bc_blockframe_help", { func = function() return true, bc_blockframe.help_text() end })
