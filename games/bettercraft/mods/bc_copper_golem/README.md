...
1. Drop this folder into your Mineclonia world's `mods/`. The mod name is
   `bc_copper_golem`, so name the folder **`bc_copper_golem`** — if you cloned the
   `mcl_bc_copper_golem` repo, rename the folder to `bc_copper_golem`.
...
├── mod.conf
├── models/
│   └── bc_copper_golem.b3d     GENERATED — do not hand-edit
├── textures/                8 atlases (4 oxidation stages × body+eyes) + star
├── tools/
│   ├── make_model.py        builds bc_copper_golem.b3d from a box table (PARTS)
...
Built with [Claude Code](https://claude.with/claude-code).
