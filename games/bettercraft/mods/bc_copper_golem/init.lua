--------------------------------------------------------------------------------
-- bc_copper_golem  --  a self-contained Copper Golem for Mineclonia
--------------------------------------------------------------------------------
--
-- WHAT THIS MOD DOES
...
local sortlib = dofile(core.get_modpath("bc_copper_golem") .. "/sortlib.lua")
...
--
-- The mesh is models/bc_copper_golem.b3d, GENERATED by tools/make_model.py from
...
local MODEL = "bc_copper_golem.b3d"
...
local STAGES = {
	{ name = "normal",    body = "bc_copper_golem.png",           eyes = "bc_copper_golem_eyes.png"           },
	{ name = "exposed",   body = "exposed_bc_copper_golem.png",   eyes = "exposed_bc_copper_golem_eyes.png"   },
	{ name = "weathered", body = "weathered_bc_copper_golem.png", eyes = "weathered_bc_copper_golem_eyes.png" },
	{ name = "oxidized",  body = "oxidized_bc_copper_golem.png",  eyes = "oxidized_bc_copper_golem_eyes.png"  },
}
...
core.register_entity("bc_copper_golem:held", {
	initial_properties = {
		visual      = "wielditem",
		wield_item  = "",
...
local function show_held(self, itemname)
	...
	local obj = core.add_entity(self.object:get_pos(), "bc_copper_golem:held", itemname)
	if obj then
...
local function star_particles(pos)
	core.add_particlespawner({
		...
		texture = "bc_copper_golem_star.png",
		glow = 14,
	})
end
...
core.register_entity("bc_copper_golem:golem", golem_def)
...
function bc_copper_golem.check_summon(pos, placer)
	...
	local obj = core.add_entity(spawn_pos, "bc_copper_golem:golem", staticdata)
	if obj then
	...
	if pumpkin then
		...
		core.override_item("mcl_farming:pumpkin_face", {
			after_place_node = function(pos, placer, itemstack, pointed_thing)
				...
				bc_copper_golem.check_summon(pos, placer)
			end,
		})
	else
		core.log("warning",
			"[bc_copper_golem] mcl_farming:pumpkin_face not found; " ..
			"build-to-spawn disabled. Is mcl_farming installed?")
	end
end

core.log("action", "[bc_copper_golem] loaded")
