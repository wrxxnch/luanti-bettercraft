...
-- Implementa autocomplete, coordinates relativas (~, ^) e commands execute, particle, testfor, testforblock, setblock


-- tabela principal do mod
bc_mineclonia_commands = {}
-- bc_mineclonia_commands.execute = {}
...
-- Function for atualizar o sinal de Redstone do Command Block e activate comparadores
...
-- Function for executar um command como se fosse o player/block de command
local function run_conditional_command(name, cmd_str)
    ...
    for single_cmd in cmd_str:gmatch("([^,]+)") do
        ...
        if cmd_def then
            ...
            minetest.chat_send_player(name, "Command condicional not found: " .. cmd)
        end
    end
end

-- Function auxiliar for parsear uma única coordenada
...
-- Function robusta for extrair position de argumentos
...
-- Function central de parsing for extrair execute= e execute!=
...
minetest.log("action", "[Mineclonia Commands] Mod loaded with success!")