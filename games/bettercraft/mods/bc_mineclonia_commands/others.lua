core.register_chatcommand("fallingnode", {
	params = "<block> <x> <y> <z>",
	description = "Spawna qualquer block como falling node (suporta ~)",
	privs = {server = true},
	...
	func = function(name, param)
		...
		if not nodename then
			return false, "Usage: /fallingnode <block> <x> <y> <z>"
		end
		...
		if not def then
			return false, "Block does not exist: " .. nodename
		end
...
--[[
	...
	----------------------------------------------------------------
	Command: /summon_jockey <mob1> <mob2> [<mob3> ...] [offset=x,y,z]

	Spawna uma PILHA de jockeys na position do player:
	  - mob1 = montaria da base (o que fica no ground)
	  - mob2 = monta mob1
	  - mob3 = monta mob2 (se informado)
	  - ...e assim por diante, quantos mobs you passar.

	offset=x,y,z (opcional, em nodes) substitui o cálculo automático
	where the passenger gets stuck, applied to ALL "floors"
	da pilha. Ex: summon_jockey zombie zombie offset=0,1.5,0
	Componentes vazios viram 0 (ex: offset=,1.5, equivale a 0,1.5,0).

	Baseado no default do chatcommand `spawn_mob` já existente no seu
	file, e na lógica de jockey vista em `mob_class:kill_me`
	(campos `_jockey_rider` na montaria e `jockey_vehicle` no
	passenger, usados for desmontar automaticamente quando a
	montaria morre).

	IMPORTANTE: este file assume que `mob_class:jock()` /
	`mob_class:unjock()` (chamados em `kill_me`) existem em algum
	outro lugar do mod, mas not define eles — o attach aqui é feito
	manualmente with `object:set_attach`. Se o seu mod já tiver um
	`mob_class:jock(vehicle)` que cuida de offsets/sons/animação,
	é melhor usar ele no lugar do block "monta a pilha" abaixo.

	Coloque este file junto aos outros do mod mcl_mobs (ele espera
	`core.registered_entities`, `core.add_entity`, etc. já
	disponíveis, without precisar de no require extra).
]]--
...
-- máximo da caixa). set_attach espera a position em unidades de 1/10
-- de node, por this o * 10.
...
-- Faz o parse de um token "offset=x,y,z" pro command summon_jockey.
-- Returns: tabela {x=,y=,z=} em unidades de node (será convertida pra
-- 1/10 de node depois), ou nil, error se o formato estiver errado.
-- Componentes ausentes (ex: "offset=,2,") viram 0.
local function parse_offset(token)
	local raw = token:match("^offset=(.*)$")
	if not raw then
		return nil, nil -- not é um token de offset
	end
	...
	if #parts ~= 3 then
		return nil, S("Offset invalid: @1 (use offset=x,y,z)", token)
	end
...
-- Desfaz o attach de uma pilha parcialmente montada, em caso de error
-- no meio do processo (ex: um mob no meio da lista not existe).
...
-- Resolve um nome de mob digitado without o prefixo do mod (ex: "zombie")
-- for o nome completo registrado (ex: "mobs_mc:zombie").
-- Returns: nome_resolvido, error
--   - Se `mob` já for um nome completo registrado, devolve ele mesmo.
--   - Senot, procura entre todas as entidades registradas por algo
--     terminando em ":"..mob.
--   - Se achar exatamente uma, resolve. Se achar mais de uma (nome
--     ambíguo entre mods), devolve error listando as opções. Se not
--     achar no, devolve error de "mob desconhecido".
local function resolve_mob_name(mob)
	...
	elseif #matches > 1 then
		table.sort(matches)
		return nil, S("Nome de mob ambíguo: @1 (found em: @2). "
			.. "Enter o nome completo with o prefixo do mod.",
...
core.register_chatcommand("summon_jockey", {
	...
	description = S("Spawna uma pilha de jockeys: mob1 é a montaria da base, "
		...
		.. "passenger fica preso na montaria, no lugar do cálculo "
	...
	func = function(playername, param)
		...
		-- "offset=x,y,z" (pode vir em qualquer position do command)
		local mobs = {}
		...
		if #mobs < 2 then
			return false, S("Usage: summon_jockey <mob1> <mob2> [<mob3> ...] "
				.. "[offset=x,y,z] (precisa de pelo menos 2 mobs).")
		end

		-- Resolve cada nome (with ou without prefixo de mod) ANTES de
		-- spawnar qualquer coisa, pra not deixar spawns "pela
		...
		-- Spawna cada mob da pilha na mesma position
		local objects = {}
		for i, mob in ipairs(mobs) do
		...
		for i = 1, #objects - 1 do
			...
			if custom_offset then
				...
				-- set_attach espera 1/10 de node, por this * 10
				attach_pos = {
