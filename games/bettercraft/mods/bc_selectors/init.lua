...
-- Exemplo: Command /tp customizado que aceita selectors
minetest.override_chatcommand("tp", {
    params = "<alvo> <destino>",
    description = "Teleport with selectors",
    privs = {teleport = true},
    ...
    func = function(name, param)
        ...
        if #args == 0 then
            return false, "Usage: /tp <alvo> [destino]"
        end
        ...
        if args[2]:sub(1,1) == "@" then
            ...
            if #dest == 0 then
                return false, "Destino not found."
            end
...
-- Exemplo: Command /kill que aceita selectors
